# -*- coding: utf-8 -*-
"""
OneShot Analysis Pipeline v_2_3
===============================

v_2_3 CHANGES:
  - Added Panel 3.2a "Null Learning" (Blind vs Icon paired dot plot) to Figure 3,
    using results['q2'] from analyze_shape_effect(). Placed at axes[0,2], same
    style as 3.2 / 3.2b. "Null learning" = the Blind->Icon SAD change from seeing
    the icon set before any data (no exposure yet).
  - Figure 3 expanded from 4x3 to 5x3 to fit the new panel. Layout:
      Row 0 [3.1, 3.2, 3.2a], Row 1 [3.2b, 3.2c, 3.2d],
      Row 2 [3.2e, 3.3, 3.4], Row 3 [3.5, 3.6, 3.3m], Row 4 [hidden].
  - All v_2_2 rare-mean companion panels (2.4m, 2.5m, 3.3m, 4.1m, 4.2m, 4.7m) and
    the sorted_est_rare_mean column are retained.

v_2_2 CHANGES:
  - Added "rare-mean" companion panels alongside the existing rare-min/rare-max
    panels (originals unchanged). The two true-rare estimates (true count = 1)
    are collapsed into a single rare_mean slot, removing the order-statistic
    artifact where max(rare1, rare2) is systematically inflated and min deflated.
    New panels (3 role slots: Rare-mean / Medium / Dominant):
      Fig 2 (now 3x3): 2.4m Blind vs Icon Prior (rare-mean) [2,0];
                       2.5m Prior Shift (rare-mean) [2,1].
      Fig 3 (4x3):     3.3m Mean Response by Phase (rare-mean) [3,1].
      Fig 4 (now 4x3): 4.1m Final Estimates vs True (rare-mean) [3,0];
                       4.2m Role Error by Phase (rare-mean) [3,1];
                       4.7m Estimation Bias (rare-mean) [3,2].
    Panel 4.3 already aggregates the two rare items into one Rare bar, so it has
    no companion. New blind-phase column sorted_est_rare_mean = mean(pos1, pos2).

v_2_1 CHANGES:
  - Figure 2 restructured from 3x3 to 2x3 "Prior Characterization":
    Removed panels 2.6 (Position-Free Count Match), 2.7 (Exact Match by Phase),
    2.8 (Exact Match by Role) — moved to Figure 4.
    Renumbered 2.9 (Prior Null Distribution) → 2.6.
  - Figure 4 restructured from 2x3 to 3x3 "Item Estimation & Match Analysis":
    Row 0 (A): Estimation vs truth (4.1-4.3, unchanged).
    Row 1 (B): Exact match analysis (4.4 Count Match, 4.5 Phase, 4.6 Role — from Fig 2).
    Row 2 (C): Error analysis (4.7 Bias, 4.8 Alt Errors, 4.9 Dist Fit — renumbered from 4.4-4.6).
  - Documented feedback utilization fix (positional → role-based item-level correction
    in analyze_feedback_utilization, applied locally in v_2_0).

BREAKING CHANGE from v1.x series (v7-v30):
  All item-level analyses now use role-based remapping instead of positional
  gen_est_item columns. gen_est_item1-4 are POSITIONAL (screen position 1-4,
  left to right), NOT item-based. Screen positions shuffle each non-blind trial.
  Previous versions incorrectly treated gen_est_item1 as always corresponding
  to item 1 (rare).

  SAD for non-blind phases now uses gen_sad from experiment code (position-aware,
  correct) instead of Python recomputation. Blind phase SAD uses sorted
  comparison (shape alignment): sorted(estimates) vs sorted([1,1,4,6]).

  New role-based columns: role_est_dominant, role_est_medium, role_est_rare_min,
  role_est_rare_max, role_est_rare_mean, role_est_rare_spread.
  Blind phase sorted analogs: sorted_est_pos1 through sorted_est_pos4.
...
FIGURE STRUCTURE:
  ...
  Figure 2: Prior Characterization (2x3, 6 panels: 2.1-2.6)
  Figure 3: Learning Trajectory Q2-Q5, Q5b (4x3, 12 panels)
  Figure 4: Item Estimation & Match Analysis (3x3, 9 panels: 4.1-4.9)
  Figure 5: Timing Analysis (2x3, 6 panels)
...
N_SIMULATIONS = 100000  # Monte Carlo simulation runs for null distribution tests
TOTAL_ITEMS = 12
...
def preprocess_data(df):
    ...
    # Distance from true (v_2_0: non-blind uses gen_sad from CSV; blind uses sorted shape alignment)
    def calc_dist_true(row):
        if row.get('trial_type') != 'blind_prior':
            gen_sad = row.get('gen_sad', np.nan)
            if not pd.isna(gen_sad):
                return gen_sad
        est = [row['gen_est_item1'], row['gen_est_item2'],
               row['gen_est_item3'], row['gen_est_item4']]
        if any(pd.isna(e) for e in est):
            return np.nan
        return calculate_sad(sorted(est), sorted(TRUE_DISTRIBUTION))
    ...
    # v_2_0: Role-based estimate remapping (positional → role)
    def remap_to_roles(row):
        result = {}
        ...
        if row.get('trial_type') == 'blind_prior':
            sorted_ests = sorted(ests)
            for i, val in enumerate(sorted_ests):
                result[f'sorted_est_pos{i + 1}'] = val
            # Collapsed rare slot for rare-mean companion panels: mean of the two
            # smallest sorted estimates (shape-aligned analog of role_est_rare_mean).
            result['sorted_est_rare_mean'] = np.mean([sorted_ests[0], sorted_ests[1]])
            return result

        mapping_str = row.get('count_mapping', '')
        if pd.isna(mapping_str) or not mapping_str:
            return result
        try:
            item_counts = {}
            for pair in str(mapping_str).split():
                item_id, count = pair.split(':')
                item_counts[item_id.strip()] = int(count)
        except Exception:
            return result
        ...
        rare_ests = []
        for i in range(1, 5):
            pos_item = row.get(f'gen_pos{i}_item', '')
            if pd.isna(pos_item):
                continue
            try:
                pos_item_str = str(int(float(pos_item)))
            except (ValueError, TypeError):
                pos_item_str = str(pos_item)
            if pos_item_str in ('', 'nan', 'W', 'X', 'Y', 'Z'):
                continue
            est = ests[i - 1]
            true_count = item_counts.get(pos_item_str, 0)
            if true_count == 6:
                dominant_est = est
            elif true_count == 4:
                medium_est = est
            elif true_count == 1:
                rare_ests.append(est)

        if not pd.isna(dominant_est):
            result['role_est_dominant'] = dominant_est
        if not pd.isna(medium_est):
            result['role_est_medium'] = medium_est
        if len(rare_ests) == 2:
            result['role_est_rare_min'] = min(rare_ests)
            result['role_est_rare_max'] = max(rare_ests)
            result['role_est_rare_mean'] = np.mean(rare_ests)
            result['role_est_rare_spread'] = abs(rare_ests[0] - rare_ests[1])

        return result

    role_remap = df.apply(remap_to_roles, axis=1)
    for col in ['role_est_dominant', 'role_est_medium', 'role_est_rare_min',
                'role_est_rare_max', 'role_est_rare_mean', 'role_est_rare_spread',
                'sorted_est_pos1', 'sorted_est_pos2', 'sorted_est_pos3', 'sorted_est_pos4',
                'sorted_est_rare_mean']:
        df[col] = role_remap.apply(lambda x: x.get(col, np.nan))
    ...
    for subj in df['subject_nr'].unique():
        ...
        for phase in PHASE_ORDER:
            ...
            if len(phase_row) > 0:
                ...
                # v_2_0: role-based columns
                if phase == 'blind_prior':
                    for si in range(1, 5):
                        summary[f'Blind_sorted{si}'] = row.get(f'sorted_est_pos{si}', np.nan)
                else:
                    for rcol in ['dominant', 'medium', 'rare_min', 'rare_max', 'rare_mean', 'rare_spread']:
                        summary[f'{prefix}_est_{rcol}'] = row.get(f'role_est_{rcol}', np.nan)
...
def analyze_population_prior(df, summary_df):
    ...
    # (A) Per-role Wilcoxon: Blind vs Icon (4 tests, Bonferroni alpha=0.0125)
    # Uses shape-aligned sorted blind columns vs role-based icon columns
    report("\n  (A) Blind vs Icon per role (Wilcoxon, Bonferroni alpha=0.0125):")
    results['item_blind_icon'] = {}  # keep key for backward compat
    results['role_blind_icon'] = {}
    n_role_tests = 4
    bonf_alpha = 0.05 / n_role_tests

    role_slots = [
        ('Rare (min)', 'Blind_sorted1', 'Icon_est_rare_min', 1),
        ('Rare (max)', 'Blind_sorted2', 'Icon_est_rare_max', 1),
        ('Medium',     'Blind_sorted3', 'Icon_est_medium',   4),
        ('Dominant',   'Blind_sorted4', 'Icon_est_dominant',  6),
    ]

    for slot_idx, (role_label, blind_col, icon_col, true_val) in enumerate(role_slots, 1):
        blind_vals, icon_vals = [], []
        for _, row in summary_df.iterrows():
            b = row.get(blind_col, np.nan)
            ic = row.get(icon_col, np.nan)
        ...
        if len(blind_arr) >= 3:
            ...
            bonf_p = min(w_p * n_role_tests, 1.0)
            ...
            result_entry = {
                ...
                'role': role_label, 'true': true_val
            }
            results['role_blind_icon'][role_label] = result_entry
            # Backward compat: also store by slot index
            results['item_blind_icon'][slot_idx] = result_entry
            sig = '***' if bonf_p < 0.001 else '**' if bonf_p < 0.01 else '*' if bonf_p < 0.05 else 'n.s.'
            report(f"    {role_label} (true={true_val}): "
                   f"Blind M={np.mean(blind_arr):.2f} -> Icon M={np.mean(icon_arr):.2f} "
                   f"(shift={shift:+.2f}), W={w_stat:.1f}, p_raw={w_p:.4f}, p_bonf={bonf_p:.4f} {sig}")
        else:
            report(f"    {role_label}: insufficient data (n={len(blind_arr)})")

    # (B) Per-role Friedman across ALL 4 phases (4 tests, Bonferroni)
    # Uses shape-aligned sorted blind columns + role-based Icon/T1/T2 columns
    report("\n  (B) Per-role Friedman across 4 phases (Bonferroni alpha=0.0125):")
    results['item_friedman_4phase'] = {}
    results['role_friedman_4phase'] = {}

    role_4phase = [
        ('Rare (min)', ['Blind_sorted1', 'Icon_est_rare_min', 'T1_est_rare_min', 'T2_est_rare_min'], 1),
        ('Rare (max)', ['Blind_sorted2', 'Icon_est_rare_max', 'T1_est_rare_max', 'T2_est_rare_max'], 1),
        ('Medium',     ['Blind_sorted3', 'Icon_est_medium',   'T1_est_medium',   'T2_est_medium'],   4),
        ('Dominant',   ['Blind_sorted4', 'Icon_est_dominant',  'T1_est_dominant',  'T2_est_dominant'],  6),
    ]

    for slot_idx, (role_label, col_list, true_val) in enumerate(role_4phase, 1):
        matrix = []
        for _, row in summary_df.iterrows():
            vals = [row.get(col, np.nan) for col in col_list]
        ...
        if matrix.shape[0] >= 3:
            ...
            bonf_p = min(p * n_role_tests, 1.0)
            means = [np.mean(matrix[:, i]) for i in range(4)]
            result_entry = {
                'chi2': chi2, 'p_uncorrected': p, 'p_bonferroni': bonf_p,
                'n': matrix.shape[0], 'phase_means': means, 'role': role_label
            }
            results['role_friedman_4phase'][role_label] = result_entry
            results['item_friedman_4phase'][slot_idx] = result_entry
            sig = '***' if bonf_p < 0.001 else '**' if bonf_p < 0.01 else '*' if bonf_p < 0.05 else 'n.s.'
            report(f"    {role_label}: chi2={chi2:.3f}, p_raw={p:.4f}, p_bonf={bonf_p:.4f} {sig} "
                   f"(means: {' -> '.join(f'{m:.2f}' for m in means)})")
        else:
            report(f"    {role_label}: insufficient data")

    # (C) Interaction: Friedman on Blind-to-Icon shift across 4 role slots
    # Shift = Icon role estimate - Blind sorted estimate per role slot
    report("\n  (C) Interaction: does shift magnitude differ by role? (Friedman on Icon-Blind shifts):")
    shift_role_slots = [
        ('Rare (min)', 'Blind_sorted1', 'Icon_est_rare_min'),
        ('Rare (max)', 'Blind_sorted2', 'Icon_est_rare_max'),
        ('Medium',     'Blind_sorted3', 'Icon_est_medium'),
        ('Dominant',   'Blind_sorted4', 'Icon_est_dominant'),
    ]
    shift_matrix = []
    for _, row in summary_df.iterrows():
        shifts = []
        valid = True
        for role_label, blind_col, icon_col in shift_role_slots:
            b = row.get(blind_col, np.nan)
            ic = row.get(icon_col, np.nan)
    ...
    if shift_matrix.shape[0] >= 3:
        ...
        shift_labels = [s[0] for s in shift_role_slots]
        report(f"    Mean shifts: " + ', '.join(
            f"{shift_labels[i]}={np.mean(shift_matrix[:,i]):+.2f}"
            for i in range(4)))
    else:
...
def analyze_item_learning(df, summary_df):
    ...
    for phase in PHASE_ORDER:
        ...
        if phase == 'blind_prior':
            mean_est = [phase_df[f'sorted_est_pos{i}'].mean() for i in range(1, 5)]
            abs_errors = [np.abs(phase_df[f'sorted_est_pos{i}'] - sorted(TRUE_DISTRIBUTION)[i - 1]).mean()
                          for i in range(1, 5)]
        else:
            mean_est = [phase_df['role_est_rare_min'].mean(), phase_df['role_est_rare_max'].mean(),
                        phase_df['role_est_medium'].mean(), phase_df['role_est_dominant'].mean()]
            abs_errors = [np.abs(phase_df['role_est_rare_min'] - 1).mean(),
                          np.abs(phase_df['role_est_rare_max'] - 1).mean(),
                          np.abs(phase_df['role_est_medium'] - 4).mean(),
                          np.abs(phase_df['role_est_dominant'] - 6).mean()]
        results['by_phase'][phase] = {'mean_estimates': mean_est, 'mean_abs_errors': abs_errors}
        role_labels_str = "Rare(min), Rare(max), Medium, Dominant"
        report(f"\n{label} [{role_labels_str}]:")
    ...
    if len(t2_df) > 0:
        ...
        for _, row in t2_df.iterrows():
            for role_col, true_val in [('role_est_dominant', 6), ('role_est_medium', 4),
                                        ('role_est_rare_min', 1), ('role_est_rare_max', 1)]:
                est = row.get(role_col, np.nan)
                if not pd.isna(est):
                    error = abs(est - true_val)
                    if true_val == 6: role_errors['dominant'].append(error)
                    elif true_val == 4: role_errors['medium'].append(error)
    ...
    # Bias analysis
    report("\n--- Bias (Mean estimate - True) [Rare(min), Rare(max), Medium, Dominant] ---")
    for phase in ['blind_prior', 'exposure_gen_2']:
        ...
        if len(phase_df) == 0:
            continue
        if phase == 'blind_prior':
            biases = [(phase_df[f'sorted_est_pos{i}'] - sorted(TRUE_DISTRIBUTION)[i - 1]).mean() for i in range(1, 5)]
        else:
            biases = [(phase_df['role_est_rare_min'] - 1).mean(),
                      (phase_df['role_est_rare_max'] - 1).mean(),
                      (phase_df['role_est_medium'] - 4).mean(),
                      (phase_df['role_est_dominant'] - 6).mean()]
...
# ============================================================================
# RARE ITEM ANALYSIS (v_2_0)
# ============================================================================
# Two items share true_count=1. This analysis examines whether participants
# differentiate between the two rare items, and how rare estimates converge
# across phases.

def analyze_rare_items(df, summary_df):
    """Analyze rare item pair: differentiation, trajectory, and spread convergence."""
    report("\n" + "=" * 70)
    report("RARE ITEM ANALYSIS (v_2_0)")
    report("=" * 70)
    results = {}

    # ---- (a) Within-rare differentiation: rare_min vs rare_max per phase ----
    report("\n  --- Within-Rare Differentiation (Wilcoxon: rare_min vs rare_max) ---")
    results['differentiation'] = {}
    phase_map = [
        ('Icon',  'Icon_est_rare_min',  'Icon_est_rare_max'),
        ('T1',    'T1_est_rare_min',    'T1_est_rare_max'),
        ('T2',    'T2_est_rare_min',    'T2_est_rare_max'),
    ]
    for label, col_min, col_max in phase_map:
        vals_min = summary_df[col_min].dropna()
        vals_max = summary_df[col_max].dropna()
        valid_idx = vals_min.index.intersection(vals_max.index)
        vals_min, vals_max = vals_min.loc[valid_idx], vals_max.loc[valid_idx]
        diffs = vals_max - vals_min
        non_zero = diffs[diffs != 0]
        if len(non_zero) >= 1:
            try:
                w, p = stats.wilcoxon(vals_min, vals_max)
            except ValueError:
                w, p = np.nan, np.nan
        else:
            w, p = 0, 1.0
        mean_diff = float(np.mean(diffs))
        results['differentiation'][label] = {
            'w': w, 'p': p, 'mean_diff': mean_diff,
            'mean_min': float(np.mean(vals_min)),
            'mean_max': float(np.mean(vals_max)),
            'n': len(valid_idx)
        }
        sig = interpret_p_value(p)
        report(f"  {label}: rare_min={np.mean(vals_min):.2f}, rare_max={np.mean(vals_max):.2f}, "
               f"diff={mean_diff:+.2f}, W={w}, p={p:.4f} {sig} (n={len(valid_idx)})")
    report("  Note: Both items have true count=1. Any difference reflects idiosyncratic bias, not learning.")

    # ---- (b) Rare learning trajectory: rare_mean across Icon -> T1 -> T2 ----
    report("\n  --- Rare Mean Trajectory (Icon -> T1 -> T2) ---")
    rare_mean_cols = ['Icon_est_rare_mean', 'T1_est_rare_mean', 'T2_est_rare_mean']
    valid_mask = summary_df[rare_mean_cols].notna().all(axis=1)
    valid = summary_df.loc[valid_mask, rare_mean_cols].values
    results['rare_trajectory'] = {}
    if valid.shape[0] >= 5:
        for i, label in enumerate(['Icon', 'T1', 'T2']):
            results['rare_trajectory'][f'{label}_mean'] = float(np.mean(valid[:, i]))
            report(f"  {label}: rare_mean = {np.mean(valid[:, i]):.2f} (true=1)")

        try:
            chi2, p = stats.friedmanchisquare(valid[:, 0], valid[:, 1], valid[:, 2])
            results['rare_trajectory']['friedman'] = {'chi2': chi2, 'p': p, 'n': valid.shape[0]}
            report(f"  Friedman chi2={chi2:.2f}, p={p:.4f} {interpret_p_value(p)} (n={valid.shape[0]})")
        except Exception:
            results['rare_trajectory']['friedman'] = None
            report("  Friedman test failed")
    else:
        report(f"  Insufficient data (n={valid.shape[0]})")

    # ---- (c) Rare spread convergence: |rare_max - rare_min| across phases ----
    report("\n  --- Rare Spread Convergence (|rare_max - rare_min|) ---")
    spread_cols = ['Icon_est_rare_spread', 'T1_est_rare_spread', 'T2_est_rare_spread']
    valid_mask_s = summary_df[spread_cols].notna().all(axis=1)
    valid_s = summary_df.loc[valid_mask_s, spread_cols].values
    results['rare_spread'] = {}
    if valid_s.shape[0] >= 5:
        for i, label in enumerate(['Icon', 'T1', 'T2']):
            results['rare_spread'][f'{label}_mean'] = float(np.mean(valid_s[:, i]))
            report(f"  {label}: spread = {np.mean(valid_s[:, i]):.2f}")

        try:
            chi2, p = stats.friedmanchisquare(valid_s[:, 0], valid_s[:, 1], valid_s[:, 2])
            results['rare_spread']['friedman'] = {'chi2': chi2, 'p': p, 'n': valid_s.shape[0]}
            report(f"  Friedman chi2={chi2:.2f}, p={p:.4f} {interpret_p_value(p)} (n={valid_s.shape[0]})")
        except Exception:
            results['rare_spread']['friedman'] = None
            report("  Friedman test failed")

        report("  Ideal: spread -> 0 as both estimates converge to 1.")
    else:
        report(f"  Insufficient data (n={valid_s.shape[0]})")
...
def analyze_exact_matches(df, summary_df):
    ...
    results = {'by_phase': {}, 'by_role': {}}
    role_labels_em = ['Rare(min)=1', 'Rare(max)=1', 'Medium=4', 'Dominant=6']

    report(f"\n--- Exact Match Rate by Phase (% of participants) ---")
    report(f"{'Phase':<8} {'Rare_min':>9} {'Rare_max':>9} {'Medium':>8} {'Dominant':>9} {'All=SAD0':>9}")
    report("-" * 56)
    ...
    for phase in PHASE_ORDER:
        ...
        if phase == 'blind_prior':
            # Position-free: sorted estimates vs sorted true
            match_rates = [
                100 * (phase_df['sorted_est_pos1'] == 1).sum() / n,
                100 * (phase_df['sorted_est_pos2'] == 1).sum() / n,
                100 * (phase_df['sorted_est_pos3'] == 4).sum() / n,
                100 * (phase_df['sorted_est_pos4'] == 6).sum() / n,
            ]
        else:
            match_rates = [
                100 * (phase_df['role_est_rare_min'] == 1).sum() / n,
                100 * (phase_df['role_est_rare_max'] == 1).sum() / n,
                100 * (phase_df['role_est_medium'] == 4).sum() / n,
                100 * (phase_df['role_est_dominant'] == 6).sum() / n,
            ]
        ...
        report(f"{label:<8} {match_rates[0]:>9.1f} {match_rates[1]:>9.1f} "
               f"{match_rates[2]:>8.1f} {match_rates[3]:>9.1f} {full_rate:>9.1f}")

    # By role (using role-based columns; blind uses sorted)
    ...
    for phase in PHASE_ORDER:
        ...
        if phase == 'blind_prior':
            dom_rate = 100 * (phase_df['sorted_est_pos4'] == 6).sum() / n
            med_rate = 100 * (phase_df['sorted_est_pos3'] == 4).sum() / n
            rare_rate = 100 * ((phase_df['sorted_est_pos1'] == 1).sum() +
                               (phase_df['sorted_est_pos2'] == 1).sum()) / (2 * n)
        else:
            dom_rate = 100 * (phase_df['role_est_dominant'] == 6).sum() / n
            med_rate = 100 * (phase_df['role_est_medium'] == 4).sum() / n
            rare_rate = 100 * ((phase_df['role_est_rare_min'] == 1).sum() +
                               (phase_df['role_est_rare_max'] == 1).sum()) / (2 * n)
...
def analyze_estimation_trends(df, summary_df):
    ...
    # For dominant: Blind uses sorted_est_pos4 (shape-aligned), others use role_est_dominant
    dom_cols = ['Blind_sorted4', 'Icon_est_dominant', 'T1_est_dominant', 'T2_est_dominant']
    for _, row in summary_df.iterrows():
        max_row, dom_row = [], []
        for prefix_idx, prefix in enumerate(['Blind', 'Icon', 'T1', 'T2']):
            ests = [row.get(f'{prefix}_est{i}', np.nan) for i in range(1, 5)]
            if not any(pd.isna(e) for e in ests):
                max_row.append(max(ests))
                dom_row.append(row.get(dom_cols[prefix_idx], np.nan))
            else:
    ...
    for phase in PHASE_ORDER:
        ...
        if len(pdf) > 0:
            if phase == 'blind_prior':
                role_in_range['dominant'].append(100 * (pdf['sorted_est_pos4'].between(4, 6)).mean())
                role_in_range['medium'].append(100 * (pdf['sorted_est_pos3'].between(4, 6)).mean())
                role_in_range['rare'].append(100 * ((pdf['sorted_est_pos1'].between(4, 6)) | (pdf['sorted_est_pos2'].between(4, 6))).mean())
            else:
                role_in_range['dominant'].append(100 * (pdf['role_est_dominant'].between(4, 6)).mean())
                role_in_range['medium'].append(100 * (pdf['role_est_medium'].between(4, 6)).mean())
                role_in_range['rare'].append(100 * ((pdf['role_est_rare_min'].between(4, 6)) | (pdf['role_est_rare_max'].between(4, 6))).mean())
    ...
    # Uses role-based columns for Icon/T1/T2, sorted columns for Blind
    report("\n  Cochran's Q per item role:")
    role_cochran_cols = {
        'Dominant': [('Blind_sorted4',), ('Icon_est_dominant',), ('T1_est_dominant',), ('T2_est_dominant',)],
        'Medium':   [('Blind_sorted3',), ('Icon_est_medium',),   ('T1_est_medium',),   ('T2_est_medium',)],
        'Rare':     [('Blind_sorted1', 'Blind_sorted2'), ('Icon_est_rare_min', 'Icon_est_rare_max'),
                     ('T1_est_rare_min', 'T1_est_rare_max'), ('T2_est_rare_min', 'T2_est_rare_max')],
    }
    for role, phase_col_groups in role_cochran_cols.items():
        binary_matrix = []
        for _, row in summary_df.iterrows():
            brow = []
            for col_group in phase_col_groups:
                ests = [row.get(c, np.nan) for c in col_group]
                if not any(pd.isna(e) for e in ests):
    ...
    results['precision'] = {}
    # Map role to (non-blind column, blind column, true value)
    precision_role_map = {
        'dominant': ('role_est_dominant', 'sorted_est_pos4', 6),
        'medium':   ('role_est_medium',   'sorted_est_pos3', 4),
    }
    for role, (nonblind_col, blind_col, true_val) in precision_role_map.items():
        phase_precision = {}
        for phase in PHASE_ORDER:
            pdf = df[df['trial_type'] == phase]
            col = blind_col if phase == 'blind_prior' else nonblind_col
            vals = pdf[col].dropna().values
            n_total = len(vals)
            if n_total > 0:
    ...
    # Uses role-based columns: sorted for Blind, role_est for others
    near_cochran_cols = {
        'dominant': (['Blind_sorted4', 'Icon_est_dominant', 'T1_est_dominant', 'T2_est_dominant'], 6),
        'medium':   (['Blind_sorted3', 'Icon_est_medium',   'T1_est_medium',   'T2_est_medium'],   4),
    }
    for role, (col_list, true_val) in near_cochran_cols.items():
        binary_mat = []
        for _, row in summary_df.iterrows():
            brow = []
            for col in col_list:
                est = row.get(col, np.nan)
                if not pd.isna(est):
    ...
    # ---- (d) Conditional probability: P(hit_dom|hit_med) and joint hits ----
    # Uses role-based columns for non-blind, sorted for blind
    ...
    for phase in PHASE_ORDER:
        ...
        if len(pdf) > 0:
            if phase == 'blind_prior':
                hit6 = (pdf['sorted_est_pos4'] == 6).values
                hit4 = (pdf['sorted_est_pos3'] == 4).values
            else:
                hit6 = (pdf['role_est_dominant'] == 6).values
                hit4 = (pdf['role_est_medium'] == 4).values
            n = len(pdf)
            n_hit6 = int(hit6.sum())
            n_hit4 = int(hit4.sum())
            ...
            report(f"  {PHASE_SHORT[phase]}: P(dom=6)={n_hit6/n:.2f} P(med=4)={n_hit4/n:.2f} "
                   f"P(both)={n_both/n:.2f} P(dom|med)={p_6g4:.2f} P(med|dom)={p_4g6:.2f}")

    # Cochran's Q on joint hits across phases
    # Uses role-based columns: sorted for Blind, role_est for others
    joint_dom_cols = ['Blind_sorted4', 'Icon_est_dominant', 'T1_est_dominant', 'T2_est_dominant']
    joint_med_cols = ['Blind_sorted3', 'Icon_est_medium', 'T1_est_medium', 'T2_est_medium']
    joint_binary = []
    for _, row in summary_df.iterrows():
        brow = []
        for dom_col, med_col in zip(joint_dom_cols, joint_med_cols):
            e_dom = row.get(dom_col, np.nan)
            e_med = row.get(med_col, np.nan)
            if not (pd.isna(e_dom) or pd.isna(e_med)):
                brow.append(1 if (e_dom == 6 and e_med == 4) else 0)
            else:
                brow.append(np.nan)
    ...
    # Uses role_est_dominant for non-blind, sorted_est_pos4 for blind
    results['dom_bins'] = {}
    ...
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type'] == phase]
        dom_col = 'sorted_est_pos4' if phase == 'blind_prior' else 'role_est_dominant'
        vals = pdf[dom_col].dropna().values
        if len(vals) > 0:
...
def analyze_feedback_utilization(df, summary_df):
    ...
    # ---- (4) Item-level correction direction (role-based) ----
    ...
    role_items = [
        ('Dominant',   'role_est_dominant',  6),
        ('Medium',     'role_est_medium',    4),
        ('Rare (min)', 'role_est_rare_min',  1),
        ('Rare (max)', 'role_est_rare_max',  1),
    ]
    for role_label, role_col, true_val in role_items:
        corrections = []
        correct_direction = 0
        for subj in subj_ids:
            t1_est = t1_df.loc[t1_df['subject_nr'] == subj, role_col]
            t2_est = t2_df.loc[t2_df['subject_nr'] == subj, role_col]
            if len(t1_est) > 0 and len(t2_est) > 0:
                ...
                if error1 > 0 and e2 < e1:
                    correct_direction += 1
                elif error1 < 0 and e2 > e1:
                    correct_direction += 1
                elif error1 == 0:
        ...
        if corrections:
            ...
            results['item_correction'][role_label] = {
...
def create_figure2(df, summary_df, results, save_path=None):
    """Figure 2: Prior Characterization (3x3)."""
    ...
    for patch in bp['boxes']:
        patch.set_facecolor(neutral_color); patch.set_alpha(0.6)
    ...
    # ---- 2.2: Blind Prior Bars (mean + SEM + individual dots) ----
    ax = axes[0, 1]
    if len(blind) > 0:
        ...
        # Plot bars in neutral color (position-based, no item identity in blind)
        ax.bar(x_items, means_b, 0.55, yerr=sems_b, color=neutral_color, alpha=0.7,
               capsize=4, edgecolor='black', linewidth=0.5, label='Blind estimate')
        # Individual dots
        rng_j = np.random.default_rng(42)
        for i in range(4):
            vals = blind[f'gen_est_item{i+1}'].dropna().values
            jx = rng_j.uniform(-0.18, 0.18, size=len(vals))
            ax.scatter(x_items[i] + jx, vals, s=12, alpha=0.4, color='#546E7A',
    ...
    ax.text(0.5, -0.18, 'Note: blind phase has no item identity. Bars show position-based estimates only.',
    ...
    # ---- 2.5: Blind vs Icon Mean Response per Role (shape-aligned) ----
    ax = axes[1, 0]
    x_items = np.arange(4)
    bar_w = 0.3
    icon = df[df['trial_type'] == 'prior_icon']
    role_labels_fig2 = ['Rare\n(min)', 'Rare\n(max)', 'Medium', 'Dominant']
    true_by_role = [1, 1, 4, 6]  # sorted TRUE_DISTRIBUTION aligned to role slots

    # Blind: sorted estimates (shape-aligned); Icon: role-based estimates
    blind_cols_fig2 = ['sorted_est_pos1', 'sorted_est_pos2', 'sorted_est_pos3', 'sorted_est_pos4']
    icon_cols_fig2 = ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant']

    for phase_idx, (phase_df, phase_label, color, offset, cols) in enumerate([
        (blind, 'Blind (sorted)', COLORS['blind_prior'], -bar_w/2, blind_cols_fig2),
        (icon, 'Icon (role)', COLORS['prior_icon'], bar_w/2, icon_cols_fig2),
    ]):
    ...
    ax.scatter(x_items, true_by_role, marker='*', s=200, color=COLORS['true'],
               zorder=10, label='True', edgecolor='black')
    ax.axhline(y=3, color='gray', ls='--', lw=1, alpha=0.5, label='Uniform (3)')
    ax.set_xticks(x_items); ax.set_xticklabels(role_labels_fig2)
    ax.set_ylabel('Mean Estimate (red dash = median)')
    ax.set_title('2.4 Blind vs Icon Prior (shape-aligned)')
    ax.legend(fontsize=6, loc='upper left'); ax.set_ylim(0, 9)

    # Annotate role-level Wilcoxon results (Blind vs Icon)
    role_bi = q1.get('role_blind_icon', {})
    if not role_bi:
        role_bi = q1.get('item_blind_icon', {})  # backward compat
    if role_bi:
        for slot_idx, role_label in enumerate(['Rare (min)', 'Rare (max)', 'Medium', 'Dominant']):
            idata = role_bi.get(role_label, role_bi.get(slot_idx + 1, {}))
            if idata.get('p_bonferroni') is not None:
                ...
                if sig:
                    ...
                    ax.text(slot_idx, max_h + 0.6, sig, ha='center', fontsize=10,
    ...
    if interaction.get('p_value') is not None:
        stats_lines.append(f"Interaction (shift x role):")
        stats_lines.append(f"\u03c7\u00b2={interaction['chi2']:.2f}, p={interaction['p_value']:.3f} {interpret_p_value(interaction['p_value'])}")
    # Role-level Wilcoxon summary
    sig_roles = [k for k, v in role_bi.items() if isinstance(k, str) and v.get('p_bonferroni', 1) < 0.05]
    if sig_roles:
        stats_lines.append(f"Wilcoxon sig (Bonf): {', '.join(sig_roles)}")
    elif role_bi:
        stats_lines.append(f"Wilcoxon: none sig (Bonf)")
    if stats_lines:
        add_stats_text(ax, '\n'.join(stats_lines), loc='upper right', fontsize=5.5)
    # Shape alignment note
    ax.text(0.5, -0.15, 'Note: Blind uses ascending-sorted estimates (shape alignment).',
            transform=ax.transAxes, fontsize=5.5, ha='center', style='italic', color='gray')
    ...
    if len(blind) > 0 and len(icon) > 0:
        ...
        true_shifts = [t - 3 for t in true_by_role]  # shift needed from uniform
    ...
    ax.set_xticks(x_items); ax.set_xticklabels(role_labels_fig2)
    ax.set_ylabel('Mean Shift (Icon role - Blind sorted)')
    ax.set_title('2.5 Prior Shift: Icon vs Blind (shape-aligned)')
    ax.legend(fontsize=6, loc='upper left')
    ...
    ax.set_title('2.6 Prior Test: Blind & Icon vs Null')

    # ====================================================================
    # Rare-mean companion panels (collapse rare_min/rare_max -> rare_mean)
    # 3 role slots: Rare (mean) / Medium / Dominant. Removes the
    # order-statistic artifact in the rare_min/rare_max split above.
    # ====================================================================
    icon = df[df['trial_type'] == 'prior_icon']
    role_labels_3 = ['Rare\n(mean)', 'Medium', 'Dominant']
    true_by_role_3 = [1, 4, 6]
    cols3_blind = ['sorted_est_rare_mean', 'sorted_est_pos3', 'sorted_est_pos4']
    cols3_role = ['role_est_rare_mean', 'role_est_medium', 'role_est_dominant']
    x3 = np.arange(3)

    # ---- 2.4m: Blind vs Icon Mean Response (rare-mean) ----
    ax = axes[2, 0]
    bar_w = 0.3
    for phase_df, phase_label, color, offset, cols in [
        (blind, 'Blind (sorted)', COLORS['blind_prior'], -bar_w/2, cols3_blind),
        (icon, 'Icon (role)', COLORS['prior_icon'], bar_w/2, cols3_role),
    ]:
        means = [phase_df[c].dropna().mean() for c in cols]
        sems = [safe_sem(phase_df[c].dropna()) for c in cols]
        medians = [phase_df[c].dropna().median() for c in cols]
        ax.bar(x3 + offset, means, bar_w, yerr=sems, color=color, alpha=0.8,
               capsize=3, edgecolor='black', linewidth=0.5, label=f'{phase_label} (mean)')
        ax.scatter(x3 + offset, medians, marker='_', color='red', s=80,
                   zorder=10, linewidths=2)
    ax.scatter(x3, true_by_role_3, marker='*', s=200, color=COLORS['true'],
               zorder=10, label='True', edgecolor='black')
    ax.axhline(y=3, color='gray', ls='--', lw=1, alpha=0.5, label='Uniform (3)')
    ax.set_xticks(x3); ax.set_xticklabels(role_labels_3)
    ax.set_ylabel('Mean Estimate (red dash = median)')
    ax.set_title('2.4m Blind vs Icon Prior (rare-mean)')
    ax.legend(fontsize=6, loc='upper left'); ax.set_ylim(0, 9)
    ax.text(0.5, -0.15, 'Note: two rare items collapsed to rare-mean; '
            'Blind uses ascending-sorted estimates.',
            transform=ax.transAxes, fontsize=5.5, ha='center', style='italic', color='gray')

    # ---- 2.5m: Per-role shift (rare-mean) ----
    ax = axes[2, 1]
    if len(blind) > 0 and len(icon) > 0:
        blind_means = [blind[c].dropna().mean() for c in cols3_blind]
        icon_means = [icon[c].dropna().mean() for c in cols3_role]
        shifts = [ic - bl for ic, bl in zip(icon_means, blind_means)]
        true_shifts = [t - 3 for t in true_by_role_3]
        colors_shift = [COLORS['rare'], COLORS['medium'], COLORS['dominant']]
        ax.bar(x3, shifts, 0.5, color=colors_shift, alpha=0.7, edgecolor='black', linewidth=0.5)
        ax.scatter(x3, true_shifts, marker='*', s=150, color=COLORS['true'], zorder=10,
                   label='Needed shift\n(true - uniform)', edgecolor='black')
        ax.axhline(y=0, color='black', lw=0.8)
        for i, s in enumerate(shifts):
            ax.text(i, s + (0.15 if s >= 0 else -0.25), f'{s:+.2f}',
    ...
    ax.set_xticks(x3); ax.set_xticklabels(role_labels_3)
    ax.set_ylabel('Mean Shift (Icon role - Blind sorted)')
    ax.set_title('2.5m Prior Shift: Icon vs Blind (rare-mean)')
    ax.legend(fontsize=6, loc='upper left')
    _ylim = max(abs(ax.get_ylim()[0]), abs(ax.get_ylim()[1]), 3.5)
    ax.set_ylim(-_ylim, _ylim)
    ax.fill_between([-0.5, 2.5], 0, _ylim, alpha=0.03, color='green')
    ax.fill_between([-0.5, 2.5], -_ylim, 0, alpha=0.03, color='red')
    ax.text(2.4, _ylim*0.1, 'Increase', fontsize=6, color='green', ha='right', style='italic')
    ax.text(2.4, -_ylim*0.1, 'Decrease', fontsize=6, color='red', ha='right', style='italic')
...
# ============================================================================
# Panels: 3.1 SAD curve, 3.2 Total learning, 3.2a Null learning (Blind-Icon),
#   3.2b Icon-T2, 3.2c Blind-T1, 3.2d Icon-T1, 3.2e T1-T2,
#   3.3 Mean response, 3.4 Trajectories, 3.5 By prior shape, 3.6 Prior-test correlation,
#   3.3m Mean response (rare-mean)

def create_figure3(df, summary_df, results, save_path=None):
    """Figure 3: Learning trajectory analysis Q2-Q5 (5x3)."""
    ...
    # ---- 3.2a: Blind-to-Icon "Null Learning" (shape info only, no exposure) ----
    ax = axes[0, 2]
    icon_sad_a = summary_df['Icon_sad'].dropna().values
    n_pairs_a = min(len(blind_sad), len(icon_sad_a))
    rng_ja = np.random.default_rng(42)
    jitter_a = rng_ja.uniform(-0.25, 0.25, size=n_pairs_a)
    for i in range(n_pairs_a):
        c = COLORS['learning'] if blind_sad[i]>icon_sad_a[i] else (COLORS['no_learning'] if blind_sad[i]<icon_sad_a[i] else 'gray')
        ax.plot([0,1], [blind_sad[i]+jitter_a[i], icon_sad_a[i]+jitter_a[i]], 'o-', color=c, alpha=0.4, markersize=5, linewidth=1.2)
    ax.errorbar([0], [np.mean(blind_sad)], yerr=[safe_sem(blind_sad)], fmt='s', color=COLORS['blind_prior'], markersize=14, capsize=6, label=f'Blind M={np.mean(blind_sad):.1f}', zorder=10, markeredgecolor='black')
    ax.errorbar([1], [np.mean(icon_sad_a)], yerr=[safe_sem(icon_sad_a)], fmt='s', color=COLORS['prior_icon'], markersize=14, capsize=6, label=f'Icon M={np.mean(icon_sad_a):.1f}', zorder=10, markeredgecolor='black')
    ax.axhline(y=UNIFORM_SAD, color='gray', ls='--', lw=1, alpha=0.5)
    ax.set_xticks([0,1]); ax.set_xticklabels(['Blind Prior', 'Icon Prior']); ax.set_ylabel('SAD')
    ax.set_title('3.2a Null Learning'); ax.legend(fontsize=8); ax.set_xlim(-0.4, 1.4)
    ni_a = int(np.sum(blind_sad[:n_pairs_a]>icon_sad_a[:n_pairs_a]))
    nw_a = int(np.sum(blind_sad[:n_pairs_a]<icon_sad_a[:n_pairs_a]))
    ns_a = n_pairs_a - ni_a - nw_a
    q2 = results.get('q2', {})
    add_stats_text(ax, f"N={n_pairs_a}\nd={q2.get('cliffs_delta',np.nan):.2f}, p={q2.get('permutation_test',{}).get('p_value',np.nan):.3f}\n{ni_a}↑ {nw_a}↓ {ns_a}↔", loc='upper right', fontsize=7)
    ...
    _paired_panel(axes[1, 1], blind_sad, t1_sad, 'Blind', 'T1', 'Blind Prior', 'Trial 1',
    ...
    _paired_panel(axes[1, 2], icon_sad, t1_sad, 'Icon', 'T1', 'Icon Prior', 'Trial 1',
                  COLORS['prior_icon'], COLORS['exposure_gen_1'], '3.2d Icon-to-T1 Learning',
                  results.get('q3', {}))

    # ---- 3.2e: T1-to-T2 Learning (repetition, mirrors 3.2) ----
    _paired_panel(axes[2, 0], t1_sad, t2_sad, 'T1', 'T2', 'Trial 1', 'Trial 2',
                  COLORS['exposure_gen_1'], COLORS['exposure_gen_2'], '3.2e T1-to-T2 Learning',
                  results.get('q4', {}))

    # ---- 3.3: Mean Response by Phase (line plot, role-based) ----
    ax = axes[2, 1]
    x_items = np.arange(4)
    _role_cols_33 = {
        'blind_prior': ['sorted_est_pos1', 'sorted_est_pos2', 'sorted_est_pos3', 'sorted_est_pos4'],
        'prior_icon': ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant'],
        'exposure_gen_1': ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant'],
        'exposure_gen_2': ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant'],
    }
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type']==phase]
        cols = _role_cols_33[phase]
        m = [pdf[c].mean() for c in cols]
        s = [safe_sem(pdf[c]) for c in cols]
        ax.errorbar(x_items, m, yerr=s, fmt='o-', color=COLORS[phase], label=PHASE_SHORT[phase], lw=2, markersize=6, capsize=3)
    ax.plot(x_items, sorted(TRUE_DISTRIBUTION), 's--', color=COLORS['true'], lw=2, markersize=8, label='True', alpha=0.9, markeredgecolor='black')
    ax.set_xticks(x_items); ax.set_xticklabels(['Rare\n(min)', 'Rare\n(max)', 'Medium', 'Dominant'])
    ...
    _role_cols_33m = {
        ...
        'prior_icon': ['role_est_rare_mean', 'role_est_medium', 'role_est_dominant'],
        'exposure_gen_1': ['role_est_rare_mean', 'role_est_medium', 'role_est_dominant'],
        'exposure_gen_2': ['role_est_rare_mean', 'role_est_medium', 'role_est_dominant'],
    }
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type'] == phase]
        cols = _role_cols_33m[phase]
        m = [pdf[c].mean() for c in cols]
        s = [safe_sem(pdf[c]) for c in cols]
        ax.errorbar(x3, m, yerr=s, fmt='o-', color=COLORS[phase], label=PHASE_SHORT[phase],
                    lw=2, markersize=6, capsize=3)
    ax.plot(x3, [1, 4, 6], 's--', color=COLORS['true'], lw=2, markersize=8,
            label='True', alpha=0.9, markeredgecolor='black')
    ax.set_xticks(x3); ax.set_xticklabels(['Rare\n(mean)', 'Medium', 'Dominant'])
    ax.set_ylabel('Mean Estimate'); ax.set_title('3.3m Mean Response by Phase (rare-mean)')
    ax.legend(fontsize=7, ncol=3, loc='upper left'); ax.set_ylim(0, 8)

    # Hide remaining unused slot in the 4x3 grid (row 3, col 2)
    axes[3, 2].axis('off')
...
# ============================================================================
# FIGURE 4: ITEM ESTIMATION & MATCH ANALYSIS (3x3)
# ============================================================================
...
def create_figure4(df, summary_df, results, save_path=None):
    ...
    x_items = np.arange(4)
    role_labels_f4 = ['Rare\n(min)', 'Rare\n(max)', 'Medium', 'Dominant']
    true_by_role_f4 = [1, 1, 4, 6]  # sorted TRUE_DISTRIBUTION aligned to role slots

    # Column mappings for role-based access per phase
    role_cols_by_phase = {
        'blind_prior': ['sorted_est_pos1', 'sorted_est_pos2', 'sorted_est_pos3', 'sorted_est_pos4'],
        'prior_icon': ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant'],
        'exposure_gen_1': ['role_est_rare_min', 'role_est_rare_max', 'role_est_medium', 'role_est_dominant'],
    ...
    # ---- 4.1: Final Estimates vs True (by role) ----
    ax = axes[0, 0]
    t2_df = df[df['trial_type']=='exposure_gen_2']
    t2_cols = role_cols_by_phase['exposure_gen_2']
    t2_m = [t2_df[c].mean() for c in t2_cols]
    t2_s = [safe_sem(t2_df[c]) for c in t2_cols]
    ax.bar(x_items-0.2, true_by_role_f4, 0.35, color=COLORS['true'], label='True', alpha=0.8, edgecolor='black', linewidth=0.5)
    ...
    ax.set_xticks(x_items); ax.set_xticklabels(role_labels_f4); ax.set_ylabel('Count')
    ...
    for phase in PHASE_ORDER:
        ...
        errors = [np.abs(pdf[c]-t).mean() for c, t in zip(cols, true_by_role_f4)]
        sems_e = [safe_sem(np.abs(pdf[c]-t)) for c, t in zip(cols, true_by_role_f4)]
        ax.errorbar(x_items, errors, yerr=sems_e, fmt='o-', color=COLORS[phase], label=PHASE_SHORT[phase], lw=2, markersize=6, capsize=3)
    ax.set_xticks(x_items); ax.set_xticklabels(role_labels_f4); ax.set_ylabel('Mean Absolute Error')
    ...
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type']==phase]
        cols = role_cols_by_phase.get(phase, role_cols_by_phase['exposure_gen_2'])
        role_err['dominant'].append(np.abs(pdf[cols[3]]-6).mean())
        role_err['medium'].append(np.abs(pdf[cols[2]]-4).mean())
        role_err['rare'].append((np.abs(pdf[cols[0]]-1).mean()+np.abs(pdf[cols[1]]-1).mean())/2)
    ...
    if em:
        ...
        for i, (true_val, label, color) in enumerate([
            (0, f'Rare min (=1)', COLORS['rare']),
            (1, f'Rare max (=1)', '#FF6B6B'),
            (2, f'Medium (=4)', COLORS['medium']),
            (3, f'Dominant (=6)', COLORS['dominant']),
        ]):
    ...
    ax.set_title('4.6 Exact Match by Item Role')

    # ---- 4.7: Estimation Bias (by role) ----
    ax = axes[2, 0]
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type']==phase]
        if len(pdf) == 0: continue
        cols = role_cols_by_phase.get(phase, role_cols_by_phase['exposure_gen_2'])
        biases = [(pdf[c]-t).mean() for c, t in zip(cols, true_by_role_f4)]
    ...
    ax.set_xticks(x_items); ax.set_xticklabels(role_labels_f4); ax.set_ylabel('Bias (Estimate - True)')
    ax.set_title('4.7 Estimation Bias (by role)'); ax.legend(fontsize=7, ncol=2)
    ylim = ax.get_ylim()
    ...
    ax.set_ylabel('Error (sum of |e|^p)'); ax.set_title('4.8 Alternative Error Measures')
    ax.legend(fontsize=7, loc='upper right')
    ...
    if dist_fit:
        ...
        ax.set_title('4.9 Distribution Fit (lower = better)')
        ...
        ax.set_title('4.9 Distribution Fit')

    # ====================================================================
    # Row 3: Rare-mean companion panels (collapse rare_min/rare_max).
    # 3 role slots: Rare (mean) / Medium / Dominant.
    # ====================================================================
    role_labels_3 = ['Rare\n(mean)', 'Medium', 'Dominant']
    true_by_role_3 = [1, 4, 6]
    role_cols3_by_phase = {
    ...
    t2_s3 = [safe_sem(t2_df[c]) for c in t2_cols3]
    ax.bar(x3-0.2, true_by_role_3, 0.35, color=COLORS['true'], label='True', alpha=0.8,
           edgecolor='black', linewidth=0.5)
    ax.bar(x3+0.2, t2_m3, 0.35, yerr=t2_s3, color=COLORS['exposure_gen_2'], label='T2 Estimate',
           alpha=0.8, capsize=3, edgecolor='black', linewidth=0.5)
    ax.set_xticks(x3); ax.set_xticklabels(role_labels_3); ax.set_ylabel('Count')
    ...
    for phase in PHASE_ORDER:
        ...
        errors = [np.abs(pdf[c]-t).mean() for c, t in zip(cols, true_by_role_3)]
        sems_e = [safe_sem(np.abs(pdf[c]-t)) for c, t in zip(cols, true_by_role_3)]
        ax.errorbar(x3, errors, yerr=sems_e, fmt='o-', color=COLORS[phase],
                    label=PHASE_SHORT[phase], lw=2, markersize=6, capsize=3)
    ax.set_xticks(x3); ax.set_xticklabels(role_labels_3); ax.set_ylabel('Mean Absolute Error')
    ...
    for phase in PHASE_ORDER:
        ...
        if len(pdf) == 0: continue
        cols = role_cols3_by_phase.get(phase, role_cols3_by_phase['exposure_gen_2'])
        biases = [(pdf[c]-t).mean() for c, t in zip(cols, true_by_role_3)]
        ax.plot(x3, biases, 'o-', color=COLORS[phase], label=PHASE_SHORT[phase], lw=2, markersize=6)
    ax.axhline(y=0, color='black', ls='-', lw=0.5)
    ax.set_xticks(x3); ax.set_xticklabels(role_labels_3); ax.set_ylabel('Bias (Estimate - True)')
    ax.set_title('4.7m Estimation Bias (rare-mean)'); ax.legend(fontsize=7, ncol=2)
    _yl = ax.get_ylim()
    ax.fill_between([-0.5, 2.5], 0, max(_yl[1], 1), alpha=0.05, color='red')
    ax.fill_between([-0.5, 2.5], min(_yl[0], -1), 0, alpha=0.05, color='blue')
    ax.text(2.4, 0.3, 'Over', fontsize=7, color='red', ha='right', style='italic')
    ax.text(2.4, -0.3, 'Under', fontsize=7, color='blue', ha='right', style='italic')
...
def generate_results_summary(results, n_subjects=0):
    ...
    # ---- ROLE-LEVEL PHASE TESTS ----
    role_bi = results.get('q1', {}).get('role_blind_icon', results.get('q1', {}).get('item_blind_icon', {}))
    role_slot_labels = ['Rare (min)', 'Rare (max)', 'Medium', 'Dominant']
    for slot_idx, role_label in enumerate(role_slot_labels, 1):
        d = role_bi.get(role_label, role_bi.get(slot_idx, {}))
        if d.get('p_bonferroni') is not None:
            role = d.get('role', role_label)
            add('Items', f'{role}: Blind vs Icon', 'Wilcoxon (Bonferroni)',
    ...
    if interaction.get('p_value') is not None:
        shifts_str = ', '.join(f"{role_slot_labels[i]}={interaction['mean_shifts'][i]:+.2f}" for i in range(4))
        add('Items', 'Interaction: shift differs by role?', 'Friedman on shifts',
            f"chi2 = {interaction['chi2']:.3f}", interaction['p_value'], '', '', shifts_str, '')

    # Per-role Friedman across 4 phases
    role_fr = results.get('q1', {}).get('role_friedman_4phase', results.get('q1', {}).get('item_friedman_4phase', {}))
    for slot_idx, role_label in enumerate(role_slot_labels, 1):
        d = role_fr.get(role_label, role_fr.get(slot_idx, {}))
        if d.get('p_bonferroni') is not None:
            means_str = ' -> '.join(f'{m:.2f}' for m in d.get('phase_means', []))
            add('Items', f'{d.get("role", role_label)}: changes across 4 phases?', 'Friedman (Bonferroni)',
    ...
    # ---- RARE ITEMS (v_2_0) ----
    ri = results.get('rare_items', {})
    ri_diff = ri.get('differentiation', {})
    for phase_label in ['Icon', 'T1', 'T2']:
        d = ri_diff.get(phase_label, {})
        if d.get('p') is not None:
            add('Items', f'Rare within-pair diff ({phase_label})', 'Wilcoxon',
                f"W = {d['w']}", d['p'], '',
                f"min={d['mean_min']:.2f}, max={d['mean_max']:.2f}",
                '', 'Both items have true count=1; difference = idiosyncratic bias')

    ri_traj = ri.get('rare_trajectory', {}).get('friedman', {})
    if ri_traj and ri_traj.get('p') is not None:
        add('Items', 'Rare mean trajectory (Icon->T1->T2)', 'Friedman',
            f"chi2 = {ri_traj['chi2']:.3f}", ri_traj['p'], '', '', '', '')

    ri_spread = ri.get('rare_spread', {}).get('friedman', {})
    if ri_spread and ri_spread.get('p') is not None:
        add('Items', 'Rare spread convergence (Icon->T1->T2)', 'Friedman',
            f"chi2 = {ri_spread['chi2']:.3f}", ri_spread['p'], '', '', '', 'Ideal: spread -> 0')
...
def generate_email_report(results, n_subjects=0):
    ...
    # Item-level correction (role-based, v_2_0)
    ic = fb.get('item_correction', {})
    if ic:
        ln(f"")
        ln(f"  Role-based correction direction (T1 -> T2):")
        for role_label in ['Dominant', 'Medium', 'Rare (min)', 'Rare (max)']:
            d = ic.get(role_label, {})
            if d:
                ln(f"    {role_label}: {d.get('pct_correct_direction', 0):.0f}% correct direction")
...
def create_figure7(df, summary_df, results, save_path=None):
    ...
    # ---- 7.4: CDF - Dominant item (role-based) ----
    ax = axes[1, 0]
    for phase in PHASE_ORDER:
        pdf = df[df['trial_type'] == phase]
        dom_col = 'sorted_est_pos4' if phase == 'blind_prior' else 'role_est_dominant'
        vals = np.sort(pdf[dom_col].dropna().values)
        if len(vals) > 0:
...
def create_figure8(df, summary_df, results, save_path=None):
    ...
    # Build per-participant category sequence (dominant role: sorted for Blind, role-based for others)
    dom_alluvial_cols = ['Blind_sorted4', 'Icon_est_dominant', 'T1_est_dominant', 'T2_est_dominant']
    categories = []
    for _, row in summary_df.iterrows():
        cat_row = []
        for col in dom_alluvial_cols:
            est = row.get(col, np.nan)
            if pd.isna(est):
                cat_row.append(-1)
...
def run_analysis(source=None, use_gui=False, output_dir=None,
    ...
    report(f"Version 2.2 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report("=" * 70)
    ...
    results['rare_items'] = analyze_rare_items(df, summary_df)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='OneShot Experiment Analysis v2.1',
