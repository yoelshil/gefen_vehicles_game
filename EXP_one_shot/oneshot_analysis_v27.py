# -*- coding: utf-8 -*-
"""
OneShot Experiment - Comprehensive Analysis (Version 12.0, v27)
==========================================================
...
def save_individual_panels(fig, fig_name, output_dir, timestamp, dpi=150):
    ...
    Best-effort: panel id is taken from the axis title (first token, dots->underscores),
    empty/colorbar axes are skipped, and each save is wrapped in try/except so one bad
    panel (e.g. a rich table or Hebrew RTL panel) can't abort the run.
    """
    ...
    for i, ax in enumerate(fig.get_axes()):
        if not (ax.has_data() or ax.get_title()):   # skip empty/colorbar axes
            continue
        title = ax.get_title()
        ...
        try:
            fig.savefig(out, dpi=dpi, bbox_inches=bbox_in, pad_inches=0.1)
...
def run_analysis(source=None, use_gui=False, output_dir=None,
    ...
    report(f"Version 27.0 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report("=" * 70)
