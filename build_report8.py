...
def move_slide(prs, old_index, new_index):
    """Move a slide so it sits at new_index in the slide order."""
    sldIdLst = prs.slides._sldIdLst
    ids = list(sldIdLst)
    sld = ids[old_index]
    sldIdLst.remove(sld)
...
def add_slide_safe(prs, slide_layout):
    """Append a slide using a guaranteed-unique partname.

    python-pptx's Slides.add_slide derives the partname from len(sldIdLst)+1, which
    collides with an existing slideN.xml once an earlier slide has been removed
    (producing a duplicate zip entry and corrupting the deck). We instead pick
    max(existing slide index)+1.
    """
    pres_part = prs.part
    package = pres_part.package
    nums = []
    for p in package.iter_parts():
        m = re.match(r"/ppt/slides/slide(\d+)\.xml$", str(p.partname))
        if m:
            nums.append(int(m.group(1)))
    n = (max(nums) + 1) if nums else 1
    partname = PackURI("/ppt/slides/slide%d.xml" % n)
    slide_part = SlidePart.new(partname, package, slide_layout.part)
    rId = pres_part.relate_to(slide_part, RT.SLIDE)
    prs.slides._sldIdLst.add_sldId(rId)
    return slide_part.slide


def add_blank_titled_slide(prs, template_slide, title_text):
    """Add a new slide from the template's layout, keep only the title, set its text."""
    slide = add_slide_safe(prs, template_slide.slide_layout)
    title_ph = slide.shapes.title
    # Strip non-title placeholders so no "click to add" prompts remain.
    for ph in list(slide.placeholders):
        if ph is not title_ph:
            ph._element.getparent().remove(ph._element)
    if title_ph is not None:
        title_ph.text = title_text
    else:  # fallback: simple textbox title
        box = slide.shapes.add_textbox(Emu(457200), Emu(228600), Emu(11277600), Emu(685800))
        box.text_frame.text = title_text
    return slide


def add_picture_fit(slide, image_path, cx, cy, cw, ch):
    """Add a picture fitted inside the (cx,cy,cw,ch) cell, preserving aspect ratio."""
    pic = slide.shapes.add_picture(str(image_path), cx, cy, width=cw)
    if pic.height > ch:
        pic.width = int(pic.width * ch / pic.height)
        pic.height = ch
    pic.left = cx + (cw - pic.width) // 2
    pic.top = cy + (ch - pic.height) // 2
    return pic


def add_rare_mean_slide(prs, subfig_dir, template_index=8):
    """Append a 2x2 rare-mean companion slide (4.1m, 4.2m, 4.7m, 3.3m) before Summary."""
    panels = [("Fig4", "4_1m"), ("Fig4", "4_2m"), ("Fig4", "4_7m"), ("Fig3", "3_3m")]
    resolved = []
    for fig_name, panel_id in panels:
        img = find_panel_file(subfig_dir, fig_name, panel_id)
        if img is None:
            print(f"  SKIP rare-mean panel {fig_name} {panel_id} — file not found")
        resolved.append(img)
    if not any(resolved):
        print("  No rare-mean panels found; slide not added.")
        return False

    template = prs.slides[template_index]
    slide = add_blank_titled_slide(
        prs, template, "Rare-Mean Companions (4.1m · 4.2m · 4.7m · 3.3m)")

    # 2x2 grid geometry on the 12192000 x 6858000 EMU canvas.
    gx, gy, gw, gh, pad = 457200, 1828800, 11277600, 4800600, 152400
    cw, ch = (gw - pad) // 2, (gh - pad) // 2
    positions = [
        (gx, gy), (gx + cw + pad, gy),
        (gx, gy + ch + pad), (gx + cw + pad, gy + ch + pad),
    ]
    for img, (lx, ty) in zip(resolved, positions):
        if img is None:
            continue
        add_picture_fit(slide, img, lx, ty, cw, ch)
        print(f"  Rare-mean slide: added {os.path.basename(img)}")

    # Move the new (last) slide to sit immediately before the Summary slide.
    n = len(prs.slides)
    summary_idx = None
    for i, s in enumerate(prs.slides):
        if i == n - 1:
            continue  # skip the slide we just added
        if find_shape_by_text(s, "Summary"):
            summary_idx = i
            break
    if summary_idx is not None:
        move_slide(prs, n - 1, summary_idx)
        print(f"  Rare-mean slide placed before Summary (now idx {summary_idx + 1})")
    else:
        print("  Summary slide not found; rare-mean slide left at end.")
    return True
...
SUBFIGURE_MAP = {
    ...
    8: [
        ...
        ("Picture 11", "Fig10", "9_4"),  # right image (v_2_4: Timing×Role saved as Fig10)
    ],
...
def main():
    ...
    parser.add_argument("--no-rare-mean-slide", action="store_true",
                        help="Don't add the rare-mean companions slide (4.1m/4.2m/4.7m/3.3m)")
    ...
    if args.subfigures and os.path.isdir(args.subfigures):
        ...
        if not args.no_rare_mean_slide:
            print("\nAdding rare-mean companions slide:")
            add_rare_mean_slide(prs, args.subfigures)
